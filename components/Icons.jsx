import AntDesignCircleInfoIcon from '@expo/vector-icons/AntDesign';
import AntDesignHomeIcon from '@expo/vector-icons/AntDesign';
import AntDesignInfoIcon from '@expo/vector-icons/AntDesign';

export const CircleInfoIcon = (props) => (
    <AntDesignCircleInfoIcon name="infocirlce" size={24} color="white" {...props} />
)

export const HomeIcon = (props) => (
    <AntDesignHomeIcon name="home" size={24} color="white" {...props} />
)

export const InfoIcon = (props) => (
    <AntDesignInfoIcon name="info" size={24} color="white" {...props} />
)